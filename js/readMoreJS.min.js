	...
	RM.init = function (options) {
		...
		for (i = 0; i < targetLen; i++) {
			...
			if (options.numOfWords < targetContentWords - 1) {
				...
				//console.log(test);
				//target[i].appendChild(test); 
				
				
				
				
				
				/*test = '<a id="rm-more_'+ i + '"'                                               // Create the More/Less link.
					+ ' class="'+ options.linkClass +'"'
					+ ' style="cursor:pointer;" data-readmore="anchor">'
					+ options.moreLink
					+ '</a>';
					console.log(target[i].nextSibling);*/
				target[i].parentNode.insertBefore(test, target[i].nextSibling);                  // Insert the More/Less link after the target element.
				//target[i].parentNode.append(test);
		...
		$(document).ready(function(){
			...
			$('.p_').each(function() {
				p_height = p_height > $(this).height() ? p_height : $(this).height();
			});
			
			console.log('p_height > ' + p_height);
			$('.a_').each(function() {
				a_height = a_height > $(this).height() ? a_height : $(this).height();
			});
			console.log('a_height > ' + a_height);
		});
			...
			rmLink[j].onclick = function () {				
				...
				if (this.getAttribute('data-clicked') !== 'true') {
					...
					if (options.toggle !== false) {
						...
						console.log(this.parentNode.parentNode.parentNode.parentNode.getBoundingClientRect().height + 25);
						this.parentNode.parentNode.parentNode.parentNode.style.height = this.parentNode.parentNode.parentNode.parentNode.getBoundingClientRect().height + 25 + "px";
					} else {
						this.innerHTML = '';
					}
				} else {
					...
					if (this.parentNode.parentNode.parentNode.parentNode.className.includes('p_')) { // identify clicked plants or animals
						this.parentNode.parentNode.parentNode.parentNode.style.height = p_height + "px";
						console.log(p_height);
					} else { //a_ // identify clicked plants or animals
						this.parentNode.parentNode.parentNode.parentNode.style.height = a_height + "px";
					}
				}
